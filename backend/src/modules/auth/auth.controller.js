...
// ══════════════════════════════════════════════
import { supabase, supabaseAnon, createSessionClient } from '../../config/supabase.js';
...
// ─── OTP request limits (CC-3) ───────────────────────────────────────────────
//
// Runbook 2.2 is the design. The short version of why none of these count by
// IP address: campus Wi-Fi NATs an entire college behind one public address,
// so "20 per machine" would mean the first twenty students to log in each
// morning use up IIITDM Kurnool's whole quota. req.ip IS recorded below — it
// is genuinely useful when reading the table after an incident — but it is
// NEVER read by a limiter. Do not add a check on it.
//
// Three limits, in increasing scope: one email, one device, the whole
// platform. Each catches something the others miss.

// The circuit breaker (runbook 2.4). Not a user-facing limit and not tuned for
// one — it is the ceiling that turns "unbounded email spend and a burned
// sending domain" into "a known worst case per hour". Launch week across
// hundreds of colleges is a few hundred genuine OTPs in the busiest hour, so
// this sits well above anything real. If it ever fires, something is wrong.
const OTP_HOURLY_CEILING = 2000;

// Rolling windows, never calendar days. A calendar reset means the student who
// hits the cap at 11:50pm waits ten minutes and the one who hits it at 12:10am
// waits almost a day, for identical behaviour.
const OTP_WINDOW_SECONDS = 24 * 60 * 60;

// Per email. Three requests are free — the first code, plus two for a slow
// mail server — and from the fourth there is a minute between them. Ten in a
// day is already generous for an address only its owner ever asks about.
const OTP_EMAIL_FREE_REQUESTS   = 3;
const OTP_EMAIL_COOLDOWN_SECONDS = 60;
const OTP_EMAIL_DAILY_CAP       = 10;

// Per device. Higher than the email cap on purpose: one device legitimately
// serves several people — a shared library computer, or a student helping a
// friend sign up on their laptop.
const OTP_DEVICE_DAILY_CAP = 20;

// X-Device-Id is a name the browser gives ITSELF, so it is untrusted input and
// worth exactly what runbook 2.2 says it is worth: it stops accidental retry
// loops, a bug in our own code, and a lazy script. Anyone can clear their
// storage and get a fresh one in a second. It is not the defence — Turnstile
// (Block E) is.
//
// The 64-character bound is the part that is not cosmetic. Without it
// otp_requests is a free write primitive for arbitrary-size rows, which is the
// same reason loginWithPassword caps identifier length.
const DEVICE_ID_PATTERN = /^[A-Za-z0-9-]{1,64}$/;

/**
 * The device id, or null. A MISSING OR MALFORMED HEADER IS NOT AN ERROR —
 * the mobile app sends no X-Device-Id today and has to keep working exactly as
 * it does now, so "no device id" is an ordinary request that simply skips the
 * per-device cap.
 */
...
/**
 * Whole seconds until `timestamp` falls out of a `windowSeconds` window.
 *
 * Clamped to at least 1 so a student is never told to wait 0 seconds and
 * retry immediately into another 429, and at most the window so a clock skew
 * cannot produce an absurd countdown.
 */
const secondsUntilExpiry = (timestamp, windowSeconds) => {
    const expiresAt = new Date(timestamp).getTime() + windowSeconds * 1000;
    const remaining = Math.ceil((expiresAt - Date.now()) / 1000);
    return Math.max(1, Math.min(windowSeconds, remaining));
};

/**
 * How long Supabase says to wait, in whole seconds.
 *
 * GoTrue puts the remaining cooldown in the message text ("...after 42
 * seconds.") and nowhere machine-readable, so it is parsed out and clamped:
 * at least 1, because it truncates a sub-second wait to "0 seconds" and a
 * student must never be told to retry immediately into another 429; at most
 * our own cooldown, so a misconfigured project cannot hand out an absurd
 * countdown. Falls back to our cooldown when the wording changes and nothing
 * parses — a slightly long wait is a much better failure than a 500.
 */
const supabaseRetryAfterSeconds = (error) => {
    const match = /after (\d+) seconds?/i.exec(error?.message ?? '');
    if (!match) return OTP_EMAIL_COOLDOWN_SECONDS;
    return Math.max(1, Math.min(OTP_EMAIL_COOLDOWN_SECONDS, Number(match[1])));
};

/**
 * One 429 shape for all three student-facing limits.
 *
 * b, c and d deliberately share the code RATE_LIMITED. The website shows one
 * countdown; it does not need to know which of the three fired, and three
 * codes would be three branches in the UI for no behavioural difference.
 */
...
// 1. Request OTP
//
// Order matters. All four checks run BEFORE signInWithOtp, because a blocked
// request must not send an email and must not create an auth user — and with
// shouldCreateUser:true below, calling Supabase does BOTH (runbook 2.3). A
// limiter that ran afterwards would be decoration.
//
// They also run before the universities lookup, so a flood costs us one
// indexed count instead of a domain query per request. That is invisible to
// honest traffic: a request with an unknown domain never reaches the insert at
// the bottom, so it never accumulates a count of its own.
export const requestOtp = async (req, res) => {
    try {
        ...
        // Fold once, here. The ledger row at the bottom is written with this
        // exact value, so every count below keys off the same string and
        // RAHUL@iiitk.ac.in cannot get a second quota of its own.
        const normalisedEmail = String(email).trim().toLowerCase();
        const deviceId        = readDeviceId(req);

        const dayStart  = new Date(Date.now() - OTP_WINDOW_SECONDS * 1000).toISOString();
        const hourStart = new Date(Date.now() - 60 * 60 * 1000).toISOString();

        // ── a. CIRCUIT BREAKER — first, before any other work ─────────────
        //
        // Counts EVERY row in the last hour: every email, every device, every
        // address. It does not care who is asking or why, which is the whole
        // point — it catches a flood we did not anticipate and have no
        // specific rule for. Reads otp_requests_created_idx.
        const { count: hourlyCount, error: breakerError } = await supabase
            .from('otp_requests')
            .select('*', { count: 'exact', head: true })
            .gte('created_at', hourStart);

        // Fail CLOSED, like the lockout read in loginWithPassword. If we
        // cannot count, we cannot promise a ceiling, and an uncountable flood
        // is the exact scenario this exists for. In practice an unreadable
        // otp_requests means the database is down, and signInWithOtp would
        // have failed a few lines later anyway.
        if (breakerError) return mapDbError(res, breakerError);

        if ((hourlyCount ?? 0) >= OTP_HOURLY_CEILING) {
            // Deliberately shouty. This should never fire in normal use, so if
            // it appears in the log something is wrong and this line is how we
            // find out. Do not soften it to console.warn.
            console.error(
                '🚨🚨🚨 [OTP CIRCUIT BREAKER TRIPPED] 🚨🚨🚨 ' +
                `${hourlyCount} OTP requests in the last hour, ceiling is ${OTP_HOURLY_CEILING}. ` +
                'ALL OTP SENDING IS NOW REFUSED until the hourly count drops. ' +
                'Check for a flood, a retry loop, or a misconfigured Turnstile before raising the ceiling.'
            );

            // 503, not 429: this is not the student's fault and there is
            // nothing they can do differently. No retry_after_seconds either —
            // we genuinely do not know when it clears.
            return sendError(res, 503, 'SERVICE_BUSY', {
                message: 'We are temporarily unable to send login codes. Please try again in a few minutes.',
            });
        }

        // ── b + c. Per-email cooldown and daily cap ───────────────────────
        //
        // One query answers both. Newest-first and capped at the daily limit,
        // so row 0 is the most recent request (the cooldown) and the last row
        // is the 10th-newest (the one whose expiry frees a slot).
        //
        // NOTE: this filters on `email`, not `lower(email)`. The stored value
        // is always the folded one written below, so the result is correct —
        // but migration 008's index is on lower(email) and PostgREST cannot
        // emit that predicate, so this does not use it. See the note in my
        // handover: it wants a plain (email, created_at desc) index.
        const { data: emailRowsRaw, error: emailError } = await supabase
            ...
            .limit(OTP_EMAIL_DAILY_CAP);

        if (emailError) return mapDbError(res, emailError);

        const emailRows = emailRowsRaw || [];

        // b. Cooldown. The first three are free — the first code plus two
        // retries for a slow mail server — and from the fourth there must be a
        // minute since the LAST one.
        if (emailRows.length >= OTP_EMAIL_FREE_REQUESTS) {
            const lastRequestedAt = new Date(emailRows[0].created_at).getTime();

            if (Date.now() - lastRequestedAt < OTP_EMAIL_COOLDOWN_SECONDS * 1000) {
                return sendRateLimited(
                    res,
                    secondsUntilExpiry(emailRows[0].created_at, OTP_EMAIL_COOLDOWN_SECONDS),
                );
            }
        }

        // c. Daily cap, rolling. retry_after_seconds is computed from the
        // OLDEST ROW THAT STILL COUNTS — the 10th-newest — because that is the
        // one whose expiry drops the count to 9 and frees a slot. With exactly
        // ten rows that is the oldest row in the window; with more, it is the
        // honest answer and the true oldest would be a lie the student would
        // find out about by getting a second 429.
        if (emailRows.length >= OTP_EMAIL_DAILY_CAP) {
            const oldestCounted = emailRows[emailRows.length - 1];
            return sendRateLimited(
                res,
                secondsUntilExpiry(oldestCounted.created_at, OTP_WINDOW_SECONDS),
            );
        }

        // ── d. Per-device daily cap ───────────────────────────────────────
        //
        // Skipped entirely when there is no device id. That is not a loophole
        // being tolerated by accident — the mobile app sends no header and
        // must keep working, and a script never had a browser to send one
        // from. Runbook 2.4 is explicit that Turnstile, not this, is what
        // stops a determined attacker. Reads otp_requests_device_created_idx.
        if (deviceId) {
            const { data: deviceRowsRaw, error: deviceError } = await supabase
                ...
                .gte('created_at', dayStart)
                .order('created_at', { ascending: false })
                .limit(OTP_DEVICE_DAILY_CAP);

            if (deviceError) return mapDbError(res, deviceError);

            const deviceRows = deviceRowsRaw || [];

            if (deviceRows.length >= OTP_DEVICE_DAILY_CAP) {
                const oldestCounted = deviceRows[deviceRows.length - 1];
                return sendRateLimited(
                    res,
                    secondsUntilExpiry(oldestCounted.created_at, OTP_WINDOW_SECONDS),
                );
        ...
        //
        // Forwarded verbatim, and checked by nobody on this side. Verification
        // happens between Supabase and Cloudflare using a SECRET KEY that lives
        // only in the Supabase dashboard — it is deliberately absent from this
        // repo, and an env var for it here would mean the flow had been
        // misunderstood. The browser only ever holds the public site key.
        //
        // This sits last, immediately before the call, because there is nothing
        // to check earlier: the four limits above are ours and run first, and
        // the token's validity is not knowable to us at all.
        //
        // A MISSING TOKEN IS NOT AN ERROR HERE. The mobile app does not send one
        // yet and Attack Protection is not enabled on the project yet, so
        // `undefined` has to behave exactly as it does today. Supabase begins
        // rejecting tokenless requests the moment that dashboard setting is
        // flipped — which is precisely why this code ships first and the setting
        // is turned on second, with no window where the live site is broken.
        const captcha =
        ...
        //
        // On the ANON client, not the service-role one: GoTrue skips captcha
        // for service-role callers, so minting here is the only way the token
        // above is actually checked. See config/supabase.js for the measurement.
        ...
        // A captcha rejection is something the student can act on — reload the
        // widget and retry — so it must not fall through to the generic 500 at
        // the bottom, which tells them nothing.
        //
        // GoTrue sets code 'captcha_failed' and a message that names the
        // captcha ("captcha protection: request disallowed (no captcha_token
        // found)"). The code is the reliable signal; the message match is kept
        // as a fallback for older GoTrue builds that set no code.
        //
        // Checked BEFORE the 429 branch: a captcha failure reported as a rate
        // limit would tell the student to wait a minute, when what they actually
        // need is to try again straight away.
        if (error?.code === 'captcha_failed' || /captcha/i.test(error?.message ?? '')) {
            return sendError(res, 400, 'CAPTCHA_FAILED', {
                message: 'We could not verify that you are a real visitor. Please reload the page and try again.',
            });
        }

        // Supabase enforces its OWN per-address cooldown on top of the four
        // checks above ([auth.email] max_frequency in supabase/config.toml) and
        // answers 429 over_email_send_rate_limit when it fires. That is a rate
        // limit, not a fault. Throwing it into the catch below reported it as
        // "500 Internal server error while sending OTP", which is how a
        // one-second local cooldown came to look exactly like the backend being
        // down — and it hid the fact that our own limiter had never run, because
        // the ledger row below is only written once Supabase accepts the send.
        if (error?.status === 429) {
            return sendRateLimited(res, supabaseRetryAfterSeconds(error));
        }

        if (error) throw error;

        // ── Record it. Only now: all four checks passed AND Supabase accepted
        // the request, so this row represents an email that was actually sent.
        //
        // ip_address is stored and never read by a limiter — runbook 2.2, the
        // NAT trap. It is here so that "1,400 requests from one address in an
        // hour" is a question the table can answer later.
        //
        // A failed write is logged and swallowed. The student's code is
        // already in their inbox; failing their login because our audit row
        // did not land would be a worse outcome than an undercount.
        ...
        if (ledgerError) {
            console.error('[otp] could not record request in otp_requests', {
                email: normalisedEmail,
                device_id: deviceId,
